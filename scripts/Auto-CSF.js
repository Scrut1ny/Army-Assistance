(async () => {
    ...
    for (let m = 2; m <= maxO; m++)
        for (let o = 0; o < m; o++)
            pool.push(new Uint8Array(Array.from({
                length: Q
            }, (_, i) => ((i + o) % m) + 1)));
    for (let b = 1; b <= 5; b++)
        pool.push(new Uint8Array(Array.from({
            length: Q
        }, (_, i) => (Math.floor(i / b) % maxO) + 1)));
    for (let s = 0; s < 20; s++)
        pool.push(new Uint8Array(Array.from({
            length: Q
        }, (_, j) => ((s * 7 + j * 13 + s * j * 3) % opts[j]) + 1)));
    ...
    while (sols.length > 1) {
        ...
        const dp = Array.from({
            length: Q
        }, (_, i) => Math.min(flat[bestP * Q + i], opts[i]));
